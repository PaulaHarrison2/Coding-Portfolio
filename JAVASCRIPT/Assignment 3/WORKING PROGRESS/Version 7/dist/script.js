...
function askYesNoQuestion(index) {
  ...
  if (answer.toLowerCase() === "yes") {
    watchList.push(series.title[index]);
...
// Function to ask if user wants to add their own recommendation
function customSeries() {
  const extraSeries = prompt("Would you like to add your own recommendation? Yes or No?");
  watchList.push(extraSeries);
  alert(`${extraSeries} has been added to you Watch List`);
}
...
for (let index = 0; index < series.title.length; index++) {
  ...
  //Ask if user wants to add their own recommendation
  const extraSeries = prompt("Would you like to add your own recommendation? Yes or No?");
